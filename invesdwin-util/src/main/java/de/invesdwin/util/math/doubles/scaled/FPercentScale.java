...
@Immutable
public enum FPercentScale implements IFDoubleScale<FPercent, FPercentScale> {
    RATE("Rate", 0, ""),
    ...
    FPercentScale(final String text, final int scale, final String symbol) {
        this.text = text;
        this.scale = scale;
        this.symbol = symbol;
    }
    ...
    @Override
    public int getDefaultDecimalDigits(final FPercent parent) {
        return FPercentScale.PERMILLE.getScale() - getScale() + 1;
    }

    @Override
    public String getFormat(final FPercent parent, final boolean withSymbol, final int decimalDigits,
