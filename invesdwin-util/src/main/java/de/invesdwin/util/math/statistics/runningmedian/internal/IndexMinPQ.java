 ...
 * <em>increase-key</em> operations take logarithmic time. The <em>is-empty</em>, <em>size</em>, <em>min-index</em>,
 * <em>min-key</em>, and <em>key-of</em> operations take constant time. Construction takes time proportional to the
 * specified capacity.
 * <p>
 ...
 */
@NotThreadSafe
public class IndexMinPQ<Key extends Comparable<Key>> implements Iterable<Entry<Integer, Key>>, Cloneable {
     ...
     *             if this priority queue is empty
     */
    public Entry<Integer, Key> delMin() {
        if (n == 0) {
            throw new java.util.NoSuchElementException("Priority queue underflow");
        }
        final int min = pq[1];
     ...
     */
    @Override
    public IndexMinPQHeapIterator<Key> iterator() {
        return new IndexMinPQHeapIterator<Key>(clone());
    }

    @SuppressWarnings("unchecked")
    @Override
    public IndexMinPQ<Key> clone() {
        try {
            final IndexMinPQ<Key> clone = (IndexMinPQ<Key>) super.clone();
            clone.maxN = maxN;
            clone.n = n;
            clone.pq = pq.clone();
            clone.qp = qp.clone();
