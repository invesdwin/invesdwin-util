 ...
 */
//CHECKSTYLE:OFF taken from jide-oss
@Immutable
public final class BigDecimalMathUtils {
    ...
    public static BigDecimal sum(final List<BigDecimal> numbers) {
        ...
        for (final BigDecimal bigDecimal : numbers) {
     ...
     */
    public static BigDecimal mean(final List<BigDecimal> numbers, final MathContext context) {
        final BigDecimal sum = sum(numbers);
     ...
     */
    public static BigDecimal min(final List<BigDecimal> numbers) {
     ...
     */
    public static BigDecimal max(final List<BigDecimal> numbers) {
     ...
     * 
     * @param numbers
     *            the numbers to calculate the standard deviation.
     * @param biasCorrected
     *            true if variance is calculated by dividing by n - 1. False if by n. stddev is a sqrt of the variance.
     * @param context
     *            the MathContext
     * @return the standard deviation
     */
    public static BigDecimal stddev(final List<BigDecimal> numbers, final boolean biasCorrected,
     ...
     */
    public static BigDecimal var(final List<BigDecimal> numbers, final boolean biasCorrected, final MathContext context) {
        final int n = numbers.size();
        if (n == 0) {
        ...
        final BigDecimal mean = mean(numbers, context);
        final List<BigDecimal> squares = new ArrayList<BigDecimal>();
        for (final BigDecimal number : numbers) {
            final BigDecimal XminMean = number.subtract(mean);
            squares.add(XminMean.pow(2, context));
        }
        final BigDecimal sum = sum(squares);
     ...
     */
    public static BigDecimal sqrt(final BigDecimal number) {
        int digits; // final precision
        BigDecimal numberToBeSquareRooted;
        BigDecimal iteration1;
        BigDecimal iteration2;
        final BigDecimal temp1 = null;
        final BigDecimal temp2 = null; // temp values

        int extraPrecision = number.precision();
        final MathContext mc = new MathContext(extraPrecision, RoundingMode.HALF_UP);
        numberToBeSquareRooted = number; // bd global variable
        final double num = numberToBeSquareRooted.doubleValue(); // bd to double
        ...
        final MathContext n = new MathContext(startPrecision, mc.getRoundingMode());
     ...
     * @return
     */
    private static BigDecimal sqrtProcedure(MathContext mc, final int digits, final BigDecimal numberToBeSquareRooted,
