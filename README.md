...
#### `ISerde` 
Serde stands for Serializer/Deserializer. They provider simplified and fast conversion from/to bytes for value objects. Preferably use the `fromBuffer`/`toBuffer` methods with sliced buffers instead of `fromBytes`/`toBytes` so that zero-copy/zero-allocation pipelines can be built. There are implementations available for most simple types. [Simple Binary Encoding](https://github.com/real-logic/simple-binary-encoding) can be used to generate code for complex types that can be wrapped by `ISerde` classes.
