...
#### `IMemoryBuffer`
When addressing memory larger than 31 bits with `int`, this interface allows you to address 63 bits using `long`. It is also possible to move the base pointer of an IByteBuffer around so that different segments can be addressed within an 63 bit address space. This allows one to use `ISerde` from an `IMemoryBuffer` using `memoryBuffer.asByteBufferFrom(address)`. This is especially helpful for memory mapped files and off heap memory larger than 2gb.
#### `ISerde` 
