...
@Immutable
public abstract class ABaseSpinWait {
    ...
    protected static MethodHandle determineOnSpinWait() {
        ...
        try {
            ...
            if (onSpinWait != null) {
                ...
                final Method noop = Reflections.findMethod(ABaseSpinWait.class, "noop");
    ...
    protected boolean awaitFulfill(final ISpinWaitCondition condition, final Instant waitingSince) throws Exception {
        return awaitFulfill(condition, waitingSince.longValue());
    }

    protected boolean awaitFulfill(final ISpinWaitCondition condition, final long waitingSinceNanos) throws Exception {
        return awaitFulfill(condition, waitingSinceNanos, Duration.ONE_YEAR);
    }

    protected boolean awaitFulfill(final ISpinWaitCondition condition, final Instant waitingSince,
            final Duration maxWait) throws Exception {
        return awaitFulfill(condition, waitingSince.longValue(), maxWait);
    }

    protected boolean awaitFulfill(final ISpinWaitCondition condition, final long waitingSinceNanos,
            final Duration maxWait) throws Exception {
        return awaitFulfill(condition, waitingSinceNanos, maxWait.nanosValue());
    }

    protected boolean awaitFulfill(final ISpinWaitCondition condition, final long waitingSinceNanos,
            final long maxWaitTime, final TimeUnit maxWaitUnit) throws Exception {
        return awaitFulfill(condition, waitingSinceNanos, TimeUnit.NANOSECONDS.convert(maxWaitTime, maxWaitUnit));
    }
    ...
    protected boolean awaitFulfill(final ISpinWaitCondition condition, final long waitingSinceNanos,
        ...
        long nanosRemaining = maxWaitNanos;
