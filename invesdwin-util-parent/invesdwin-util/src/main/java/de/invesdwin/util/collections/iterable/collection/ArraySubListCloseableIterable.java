...
public class ArraySubListCloseableIterable<E> implements ICloseableIterable<E>, IFastToListProvider<E> {
    ...
    static {
        ...
        SUBLIST_SIZE_FIELD_OFFSET = UnsafeApi.objectFieldOffset(sublistSizeField);
        final Field sublistOffsetField = Reflections.findField(SUBLIST_CLASS, "offset");
        SUBLIST_OFFSET_FIELD_OFFSET = UnsafeApi.objectFieldOffset(sublistOffsetField);
        ...
        if (sublistRootField != null) {
            ...
            SUBLIST_PARENT_FIELD_OFFSET = UnsafeApi.objectFieldOffset(sublistRootField);
        } else {
            //before java 11
            final Field sublistParentField = Reflections.findField(SUBLIST_CLASS, "parent");
            SUBLIST_PARENT_FIELD_OFFSET = UnsafeApi.objectFieldOffset(sublistParentField);
        }
    }
    ...
    public ICloseableIterator<E> iterator() {
        try {
            final int size = UnsafeApi.getInt(arraySubList, SUBLIST_SIZE_FIELD_OFFSET);
            if (cachedSize != size) {
                cachedSize = size;
                List<E> parent = (List<E>) arraySubList;
                while (parent.getClass().equals(SUBLIST_CLASS)) {
                    parent = (List<E>) UnsafeApi.getReference(parent, SUBLIST_PARENT_FIELD_OFFSET);
                }
                final ArrayList<E> arrayListParent = (ArrayList<E>) parent;
                cachedArray = (E[]) UnsafeApi.getReference(arrayListParent,
                        ArrayListCloseableIterable.ARRAYLIST_ELEMENTDATA_FIELD_OFFSET);
                cachedOffset = UnsafeApi.getInt(arraySubList, SUBLIST_OFFSET_FIELD_OFFSET);
            }
