 ...
 */
@ThreadSafe
public abstract class AReadWriteLockedFastIterableDelegateMap<K, V> implements IFastIterableMap<K, V> {
    ...
    protected AReadWriteLockedFastIterableDelegateMap(final Map<K, V> delegate) {
        this.delegate = delegate;
        refreshFastIterable();
        this.lock = Locks.newReentrantReadWriteLock(getClass().getSimpleName());
    }

    protected AReadWriteLockedFastIterableDelegateMap(final Map<K, V> delegate, final IReadWriteLock lock) {
        this.delegate = delegate;
        refreshFastIterable();
        this.lock = lock;
    }

    public AReadWriteLockedFastIterableDelegateMap() {
        this.delegate = newDelegate();
        refreshFastIterable();
        this.lock = Locks.newReentrantReadWriteLock(getClass().getSimpleName());
    }

    public AReadWriteLockedFastIterableDelegateMap(final IReadWriteLock lock) {
        this.delegate = newDelegate();
        refreshFastIterable();
        this.lock = lock;
    }
    ...
    public V[] asValueArray(final V[] emptyArray) {
        lock.readLock().lock();
        try {
            final V[] valueArrayCopy = valueArray;
            if (valueArrayCopy != null) {
    ...
    public K[] asKeyArray(final K[] emptyArray) {
        lock.readLock().lock();
        try {
            final K[] keyArrayCopy = keyArray;
            if (keyArrayCopy != null) {
                return keyArrayCopy;
            }
        } finally {
            lock.readLock().unlock();
    ...
    public Entry<K, V>[] asEntryArray() {
        lock.readLock().lock();
        try {
            final Entry<K, V>[] entryArrayCopy = entryArray;
            if (entryArrayCopy != null) {
                return entryArrayCopy;
            }
        } finally {
            lock.readLock().unlock();
        }
    ...
    private final class ValuesCollection implements Collection<V>, Serializable {
        @Override
        public int size() {
            return AReadWriteLockedFastIterableDelegateMap.this.size();
        }

        @Override
        public boolean isEmpty() {
            return AReadWriteLockedFastIterableDelegateMap.this.isEmpty();
        }
