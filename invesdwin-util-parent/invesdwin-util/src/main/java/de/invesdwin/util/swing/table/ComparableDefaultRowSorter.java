...
public abstract class ComparableDefaultRowSorter<M, I> extends RowSorter<M> {
     ...
     * <code>toggleSortOrder</code> is invoked. It is still possible to sort on a column that has been marked as
     * unsortable by directly setting the sort keys. The default is true.
     *
     * @param column
    ...
    public void setSortKeys(final List<? extends SortKey> sortKeys) {
        ...
        if (sortKeys != null && sortKeys.size() > 0) {
            final int max = getModelWrapper().getColumnCount();
     ...
     * sorting. For example, assume a table row sorter is created and <code>setMaxSortKeys(2)</code> is invoked on it.
     * The user clicks the header for column 1, causing the table rows to be sorted based on the items in column 1.
     * Next, the user clicks the header for column 2, causing the table to be sorted based on the items in column 2; if
     * any items in column 2 are equal, then those particular rows are ordered based on the items in column 1. In this
     * case, we say that the rows are primarily sorted on column 2, and secondarily on column 1. If the user then clicks
     * the header for column 3, then the items are primarily sorted on column 3 and secondarily sorted on column 2.
     * Because the maximum number of sort keys has been set to 2 with <code>setMaxSortKeys</code>, column 1 no longer
     * has an effect on the order.
     * <p>
     ...
     *             if <code>max</code> &lt; 1
     */
    public void setMaxSortKeys(final int max) {
        if (max < 1) {
     ...
     * invoked). For example, if this is true and the user edits an entry the location of that item in the view may
     ...
     * already the primary sorted column; otherwise, makes the specified column the primary sorted column, with an
     * ascending sort order. If the specified column is not sortable, this method has no effect.
     *
     * @param column
    ...
    private int convertUnsortedUnfiltered(final int index) {
        if (index < 0 || index >= modelRowCount) {
            if (index >= modelRowCount && index < getModelWrapper().getRowCount()) {
                // 8160087
                if (warning8160087) {
    ...
    private void sortExistingData() {
        final int[] lastViewToModel = getViewToModelAsInts(viewToModel);
     ...
     * filter, if any, associated with this sorter. An empty <code>sortKeys</code> list indicates that the view should
    ...
    private void initializeFilteredMapping() {
        final int rowCount = getModelWrapper().getRowCount();
        int i, j;
        int excludedCount = 0;
     ...
     */
    private void createModelToView(final int rowCount) {
     ...
     */
    private void cacheSortKeys(final List<? extends SortKey> keys) {
        final int keySize = keys.size();
     ...
     * <code>ModelWrapper.getStringValueAt</code> will be used, otherwise <code>ModelWrapper.getValueAt</code> will be
     * used. It is up to subclasses, such as <code>TableRowSorter</code>, to honor this value in their
     * <code>ModelWrapper</code> implementation.
     *
     * @param column
     *            the index of the column to test, in terms of the underlying model
     * @return true if values are to be converted to strings before doing comparisons when sorting
     * @throws IndexOutOfBoundsException
     ...
     */
    private void setModelToViewFromViewToModel(final boolean unsetFirst) {
        int i;
        if (unsetFirst) {
    ...
    private int[] getViewToModelAsInts(final Row[] viewToModel) {
        if (viewToModel != null) {
            final int[] viewToModelI = new int[viewToModel.length];
     ...
     * want to sort after setting the comparator you need to explicitly invoke <code>sort</code>.
     *
     * @param column
     ...
     */
    public void setComparator(final int column, final Comparator<?> comparator) {
        checkColumn(column);
        if (comparators == null) {
    ...
    private Comparator<?> getComparator0(final int column) {
     ...
     */
    private boolean include(final int row) {
        final RowFilter<? super M, ? super I> filter = getRowFilter();
        if (filter != null) {
    ...
    private int compare(final int model1, final int model2) {
        int column;
        SortOrder sortOrder;
        Object v1, v2;
        int result;
    ...
    protected int compare(final int modelIndex1, final Object value1, final int modelIndex2, final Object value2,
            final Comparator<Object> comparator, final SortOrder sortOrder) {
        int result;
        if (value1 == null) {
            if (value2 == null) {
                result = 0;
     ...
     */
    private void insertInOrder(final List<Row> toAdd, final Row[] current) {
        int last = 0;
        int index;
        final int max = toAdd.size();
        for (int i = 0; i < max; i++) {
     ...
     * false, assume the event was dealt with and no further processing needs to happen.
     */
    private boolean shouldOptimizeChange(final int firstRow, final int lastRow) {
        if (!isTransformed()) {
            // Not transformed, nothing to do.
            return false;
        }
        if (!sorted || viewToModel.length == 0 || (lastRow - firstRow) > viewToModel.length / 10) {
    ...
    private void rowsInserted0(final int firstRow, final int lastRow) {
        final int[] oldViewToModel = getViewToModelAsInts(viewToModel);
        int i;
        final int delta = (lastRow - firstRow) + 1;
        final List<Row> added = new ArrayList<Row>(delta);
        ...
        if (added.size() > 0) {
            ...
            final Row[] lastViewToModel = viewToModel;
    ...
    private void rowsDeleted0(final int firstRow, final int lastRow) {
        final int[] oldViewToModel = getViewToModelAsInts(viewToModel);
        ...
        if (removedFromView > 0) {
            final Row[] newViewToModel = new Row[viewToModel.length - removedFromView];
            int newIndex = 0;
            int last = 0;
    ...
    private void rowsUpdated0(final int firstRow, final int lastRow) {
        final int[] oldViewToModel = getViewToModelAsInts(viewToModel);
        int i, j;
        final int delta = lastRow - firstRow + 1;
        int modelIndex;
        ...
        if (getRowFilter() == null) {
            ...
            final Row[] updated = new Row[delta];
            ...
            final Row[] intermediary = new Row[viewToModel.length - delta];
            ...
            final List<Row> updated = new ArrayList<Row>(delta);
            int newlyVisible = 0;
            int newlyHidden = 0;
            int effected = 0;
            for (i = firstRow; i <= lastRow; i++) {
            ...
            final Row[] intermediary = new Row[viewToModel.length - effected];
            for (i = 0, j = 0; i < viewToModel.length; i++) {
                modelIndex = viewToModel[i].modelIndex;
     ...
     * <code>DefaultRowSorter</code>. You normally do not interact directly with <code>ModelWrapper</code>. Subclasses
     * of <code>DefaultRowSorter</code> provide an implementation of <code>ModelWrapper</code> wrapping another model.
     * For example, <code>TableRowSorter</code> provides a <code>ModelWrapper</code> that wraps a
    ...
    protected abstract static class ModelWrapper<M, I> {
         ...
         * <code>RowFilter.Entry</code> that is passed to the <code>RowFilter</code>.
         *
         * @param row
