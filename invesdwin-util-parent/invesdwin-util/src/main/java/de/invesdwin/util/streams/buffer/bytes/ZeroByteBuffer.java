...
@NotThreadSafe
public class ZeroByteBuffer implements ICloseableByteBuffer {

    /**
     * This is also read from a closed socket.
     */
    public static final byte ZERO_BYTE = (byte) 0;
    public static final byte[] ZERO_ARRAY = new byte[] { ZERO_BYTE };

    public static final java.nio.ByteBuffer ZERO_BYTE_BUFFER = java.nio.ByteBuffer.wrap(ZERO_ARRAY);
    public static final AtomicBuffer ZERO_DIRECT_BUFFER = new UnsafeBuffer(ZERO_ARRAY);
    ...
    public void getBytes(final int index, final byte[] dst, final int dstIndex, final int length) {
        ZERO_DIRECT_BUFFER.getBytes(index, dst, dstIndex, length);
    }

    @Override
    public void getBytes(final int index, final MutableDirectBuffer dstBuffer, final int dstIndex, final int length) {
        ZERO_DIRECT_BUFFER.getBytes(index, dstBuffer, dstIndex, length);
    }
    ...
    public void getBytes(final int index, final IByteBuffer dstBuffer, final int dstIndex, final int length) {
        ...
        if (directBuffer != null) {
            ZERO_DIRECT_BUFFER.getBytes(index, directBuffer,
                    dstIndex + dstBuffer.wrapAdjustment() - directBuffer.wrapAdjustment(), length);
        } else if (dstBuffer.nioByteBuffer() != null) {
            ZERO_DIRECT_BUFFER.getBytes(index, dstBuffer.nioByteBuffer(), dstIndex + dstBuffer.wrapAdjustment(),
                    length);
        } else if (dstBuffer.byteArray() != null) {
            ZERO_DIRECT_BUFFER.getBytes(index, dstBuffer.byteArray(), dstIndex + dstBuffer.wrapAdjustment(), length);
        } else {
    ...
    public byte[] asByteArray(final int index, final int length) {
        if (index == 0 && length == ZERO_ARRAY.length) {
            return ZERO_ARRAY;
        } else {
            throw newZeroException();
        }
